    ...
    if(isset($_POST['updatestatus'])){
        ...
        if($status == 'completed'){
            ...
            if($applyUpdateAppointment){
                $fetchDetails = "SELECT CenterId,GroupId,DonorId,Quantity FROM `requesttable` WHERE RequestId = $requestId;";
                $applyFetchDetails = mysqli_query($config,$fetchDetails);
                if($applyFetchDetails){
                    $getFetchDetails = mysqli_fetch_assoc($applyFetchDetails);
                    $centerId = intval($getFetchDetails['CenterId']);
                    $groupId = intval($getFetchDetails['GroupId']);
                    $donorId = intval($getFetchDetails['DonorId']);
                    $quantity = intval($getFetchDetails['Quantity']);
                    $expiryDate = date("$expiryYear-m-d");
                    $insertInventory = "INSERT INTO `inventory`( `CenterId`, `GroupId`, `DonorId`, `Quantity`, `ExpirationDate`) VALUES ($centerId,$groupId,$donorId,$quantity,'$expiryDate')";
                    $applyInsertInventory = mysqli_query($config,$insertInventory);
                    if($applyInsertInventory){
                        $insertDonorActivity = "INSERT INTO `donoractivity`( `DonorId`, `GroupId`, `Quantity`, `DonationDate`) VALUES ($donorId,$groupId,$quantity,'$appointmentDate')";
                        $applyInsertDonorActivity = mysqli_query($config,$insertDonorActivity);
                        if($applyInsertDonorActivity){

                            header("location:./donorschedule.php");
                        }
                    }
...
<!-- Sidebar Start -->
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="./donorschedule.php" class="nav-item nav-link active"><i class="fa fa-table me-2"></i>Donors Schedule</a>
        ...
        <h2 class="text-center my-4">DONORS SCHEDULE DETAILS</h1>
	...
	<tbody>
        <?php 
            $activityQuery = "SELECT appointment.AppointmentId,appointment.RequestId,donor.DonorName,bloodgroup.GroupName,requesttable.Timings,appointment.AppointmentDate,requesttable.Quantity,bloodcenter.CenterName,appointment.AppointmentStatus FROM `appointment` INNER JOIN requesttable ON appointment.RequestId = requesttable.RequestId INNER JOIN donor ON requesttable.DonorId = donor.DonorId INNER JOIN bloodgroup ON requesttable.GroupId = bloodgroup.GroupId INNER JOIN bloodcenter ON requesttable.CenterId = bloodcenter.CenterId WHERE requesttable.RecipientId IS NULL ORDER BY requesttable.RequestDate;";
