...
<!-- Sidebar Start -->
        ...
        <div style="overflow-y: scroll;" class="tableContainer content">
            <h1 class="text-center my-4">RECIPIENTS DETAILS</h1>
	...
	<tbody>
        <?php 
            $activityQuery = "SELECT users.UserId, recipient.RecipientName,users.UserEmail,recipient.RecipientContact,recipient.RecipientAddress,city.CityName,bloodgroup.GroupName,recipient.RecipientGender,recipient.RecipientDOB,recipient.RecipientWeight FROM `recipient` INNER JOIN users ON recipient.UserId = users.UserId INNER JOIN city ON recipient.CityId = city.CityId INNER JOIN bloodgroup ON recipient.GroupId = bloodgroup.GroupId ORDER BY recipient.Recipient";
		...
		<tr>
			...
			<td>
                <a href="mailto: <?php echo $fetchActivity['UserEmail'] ?> "><?php echo $fetchActivity['UserEmail'] ?></a> 
            </td>
			<td><?php echo $fetchActivity['RecipientContact'] ?></td>
			<td><?php echo $fetchActivity['RecipientAddress'] ?></td>
			<td><?php echo $fetchActivity['CityName'] ?></td>
			<td><?php echo $fetchActivity['GroupName'] ?></td>
			<td><?php echo $fetchActivity['RecipientGender'] ?></td>
			<td><?php echo $fetchActivity['RecipientDOB'] ?></td>
			<td><?php echo $fetchActivity['RecipientWeight'] ?></td>
