...
if($_POST['type']=='search'){
 ...
 WHERE city_name LIKE '%$searchValue%' OR state_name LIKE '%$searchValue%' ORDER BY city_id DESC ";
...
if ($_POST["query"] != '') {
    ...
    $sel_query = "SELECT * FROM city
    JOIN state ON city.state_id = state.state_id
 WHERE city_id IN (" . $search_text . ") ORDER BY city_id DESC ";
} else {
    $sel_query = "SELECT * FROM city
    JOIN state ON city.state_id = state.state_id
    WHERE city.delete_status = 0";
}
