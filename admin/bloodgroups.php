<?php
    ...
    if(isset($_POST['updategroup'])){
        ...
        $updateGroupQuery = "UPDATE `bloodgroup` SET `GroupName`='$groupName',`GroupDescription`='$groupDescription' WHERE GroupId = $groupId ";
        $applyUpdateGroup = mysqli_query($config,$updateGroupQuery);
        if($applyUpdateGroup){
            header("location:./bloodgroups.php");
        }
    }
    ...
    if(isset($_POST['add'])){
        ...
        $checkBloodName = "SELECT GroupName FROM bloodgroup WHERE GroupName = '$groupName'";
        $applyCheckBlood = mysqli_query($config,$checkBloodName);
        if(mysqli_num_rows($applyCheckBlood)>0){
            $_SESSION["nameError"] = "This blood group is already in our records";
        }else{
            $insertBloodGroup = "INSERT INTO `bloodgroup`( `GroupName`, `GroupDescription`) VALUES ('$groupName','$groupDescription')";
            $applyInsertBloodGroup = mysqli_query($config,$insertBloodGroup);
            if($applyInsertBloodGroup){
                header("location:./bloodgroups.php");
            }
        }
...
<!-- Sidebar Start -->
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h2 style="opacity:0.8; font-family:Heebo;" class="modal-title">Add Blood Groups</h1>
      ...
      <div class="modal-body">
        <form role="form" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" onsubmit="return updateValidation(0);">
          <div class="mb-3">
            ...
            <input type="text" class="form-control groupname0" name="groupname" value="" onfocus="focusSetup(document.querySelector(`.groupname0`))">
          </div>
          <div class="mb-3">
            <label class="form-label">Group Description</label>
            <input type="text" class="form-control description0" name="description" value="" onfocus="focusSetup(document.querySelector(`.description0`))">
          </div>
        ...
        <div style="overflow-y: scroll;" class="tableContainer content">
            <div class="d-flex justify-content-between align-items-center mt-5 tableHeader">
                <h3 style="margin:0 !important;">BLOOD GROUPS</h5>
	...
	<tbody>
        <?php 
            $activityQuery = "SELECT * FROM `bloodgroup`";
		...
		<tr>
			...
			<td><?php echo $fetchActivity['GroupDescription'] ?></td>
			<td>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h2 style="opacity:0.8; font-family:Heebo;" class="modal-title">Update Blood Groups</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form role="form" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" onsubmit="return updateValidation(<?php echo $i ?>);">
          <input hidden id="gi" type="" name="groupid" value="<?php echo $fetchActivity['GroupId'] ?>">
          <div class="mb-3">
            <label class="form-label">Group Name</label>
            <input type="text" class="form-control groupname<?php echo $i ?>" name="groupname" value="<?php echo $fetchActivity['GroupName'] ?>" onfocus="focusSetup(document.querySelector(`.groupname<?php echo $i ?>`))">
          </div>
          <div class="mb-3">
            <label class="form-label">Group Description</label>
            <input type="text" class="form-control description<?php echo $i ?>" name="description" value="<?php echo $fetchActivity['GroupDescription'] ?>" onfocus="focusSetup(document.querySelector(`.description<?php echo $i ?>`))">
          </div>
          
          <div class="mb-3">
            <button type="submit" class="btn btn-success" name="updategroup">Update</button>
          </div>
        </form>
      </div>
    </div>
...
<a href="#" class="btn btn-lg btn-primary btn-lg-square back-to-top"><i class="bi bi-arrow-up"></i></a>
    ...
    <script>
        function updateValidation(i){
            let groupname = document.querySelector(`.groupname${i}`);
            let groupDescription = document.querySelector(`.description${i}`);
            let errorArray = [];
            ...
            if(groupDescription.value == ""){
                groupDescription.style.border = '2px solid red';
                errorArray.push(false);
            }
