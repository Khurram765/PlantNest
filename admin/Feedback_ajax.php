...
if($_POST['type']=='search'){
    ...
    $sel_query = "SELECT * FROM `feedback` WHERE `delete_status`= 0 AND (feedback_subject LIKE '%$searchValue%' OR feedback_text LIKE '%$searchValue%') ";
    ...
    if (mysqli_num_rows($run) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($run)) {
            ...
            $output .= '</a>';
            $output .= '<a class="btn btn-sm bg-danger-light" href="Feedback_edit.php?type=update&feedback_id=' . $row['feedback_id'] . '">';
