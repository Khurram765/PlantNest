<?php
    ...
    if(isset($_POST['approve'])){
        ...
        $updateRequest = "UPDATE `requesttable` SET `RequestStatus`='confirmed' WHERE RequestId = $requestId";
        $applyUpdateRequest = mysqli_query($config,$updateRequest);
        if($applyUpdateRequest){
            ...
            if($applyInsertAppointment){
                $getUserId = "SELECT users.UserId FROM requesttable INNER JOIN donor ON requesttable.DonorId = donor.DonorId INNER JOIN users ON donor.UserId = users.UserId WHERE requesttable.RequestId = $requestId";
                $applyGetUser = mysqli_query($config,$getUserId);
                if($applyGetUser){
                    $row = mysqli_fetch_assoc($applyGetUser);
                    ...
                    if($applySendNotification){
                        ...
                        header("location:./donorrequests.php");
                    }
                }
            }
        }
...
<!-- Sidebar Start -->
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="./donorrequests.php" class="nav-item nav-link active"><i class="fa fa-clock me-2"></i>Donor Requests</a>
        ...
        <h1 class="text-center my-4">DONORS REQUESTS</h1>
	...
	<tbody>
        <?php 
            $activityQuery = "SELECT requesttable.RequestId,donor.DonorName,bloodgroup.GroupName,requesttable.Timings,requesttable.RequestDate,requesttable.Quantity,bloodcenter.CenterName FROM `requesttable` INNER JOIN donor ON requesttable.DonorId = donor.DonorId INNER JOIN bloodgroup ON requesttable.GroupId = bloodgroup.GroupId INNER JOIN bloodcenter ON requesttable.CenterId = bloodcenter.CenterId WHERE requesttable.RecipientId IS NULL AND requesttable.RequestStatus = 'pending';";
