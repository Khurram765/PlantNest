...
if($_POST['type']=='search'){
    ...
    $sel_query = "SELECT * FROM plants
    JOIN categories ON plants.category_id = categories.category_id
    WHERE plants.delete_status = 0 AND (`name` LIKE '%$searchValue%' OR `species` LIKE '%$searchValue%' OR `price` LIKE '%$searchValue%') ";
...
if ($_POST["query"] != '') {
    ...
    $sel_query = "SELECT * FROM plants
    JOIN categories ON plants.category_id = categories.category_id
    WHERE plant_id IN (" . $search_text . ") ORDER BY plant_id DESC ";
} else {
    $sel_query = "SELECT * FROM plants
    JOIN categories ON plants.category_id = categories.category_id
    WHERE plants.delete_status = 0";
}
