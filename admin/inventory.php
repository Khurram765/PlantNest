...
<!-- Sidebar Start -->
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="./inventory.php" class="nav-item nav-link active"><i class="fa fa-box me-2"></i>Inventory</a>
        ...
        <h3 class="text-center my-4">BLOOD STOCK</h1>
	...
	<tbody>
        <?php 
            $activityQuery = "SELECT bloodcenter.CenterName,bloodgroup.GroupName,SUM(inventory.Quantity) AS quantity,inventory.ExpirationDate FROM `inventory` INNER JOIN bloodcenter ON inventory.CenterId = bloodcenter.CenterId INNER JOIN bloodgroup ON inventory.GroupId = bloodgroup.GroupId GROUP BY inventory.GroupId,bloodcenter.CenterId";
        ...
        <!-- Table -->
        <div style="overflow-y: scroll;" class="tableContainer content">
        <h3 class="text-center my-4">INDIVDUAL STOCK RECORD WITH EXPIRATION DATE</h1>
	...
	<tbody>
        <?php 
            $activityQuery = "SELECT bloodcenter.CenterName,bloodgroup.GroupName,inventory.Quantity,inventory.ExpirationDate FROM `inventory` INNER JOIN bloodcenter ON inventory.CenterId = bloodcenter.CenterId INNER JOIN bloodgroup ON inventory.GroupId = bloodgroup.GroupId ";
            $applyActivityQuery = mysqli_query($config,$activityQuery);
            $i = 0;
            if(mysqli_num_rows($applyActivityQuery)>0){
                while($fetchActivity = mysqli_fetch_assoc($applyActivityQuery)){
		...
		<tr>
			...
			<td><?php echo $fetchActivity['GroupName'] ?></td>
			<td><?php echo $fetchActivity['Quantity'] ?></td>
			<td><?php echo $fetchActivity['ExpirationDate'] ?></td>
		</tr>
        <?php }}?>
