...
<!-- Sidebar Start -->
	...
	<tbody>
    <?php 
            $activityQuery = "SELECT recipient.RecipientName,bloodgroup.GroupName,requesttable.Timings,requesttable.RequestDate,requesttable.Quantity,bloodcenter.CenterName FROM `requesttable` INNER JOIN recipient ON requesttable.RecipientId = recipient.Recipient INNER JOIN bloodgroup ON requesttable.GroupId = bloodgroup.GroupId INNER JOIN bloodcenter ON requesttable.CenterId = bloodcenter.CenterId WHERE requesttable.RecipientId = $recipientId AND RequestStatus = 'pending'";
