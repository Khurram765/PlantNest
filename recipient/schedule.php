...
<!-- Sidebar Start -->
        ...
        <div style="overflow-y: scroll;" class="tableContainer content">
            <h1 class="text-center my-4">COMING SCHEDULE</h1>
	...
	<tbody>
    <?php 
            $activityQuery = "SELECT recipient.RecipientName,bloodgroup.GroupName,requesttable.Timings,appointment.AppointmentDate,bloodcenter.CenterName FROM `appointment` INNER JOIN requesttable ON appointment.RequestId = requesttable.RequestId INNER JOIN recipient ON requesttable.RecipientId = recipient.recipient INNER JOIN bloodgroup ON requesttable.GroupId = bloodgroup.GroupId INNER JOIN bloodcenter ON requesttable.CenterId = bloodcenter.CenterId WHERE requesttable.RecipientId = $recipientId AND appointment.AppointmentStatus = 'coming';";
